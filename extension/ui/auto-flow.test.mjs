...
import {startTrouble} from '../core/dispatch.js';
...
for (const mode of ['fast','standard','detailed',undefined]) test(`awaits outline → writing → editor → images → export (${mode || 'legacy'})`,async()=>{
 ...
 let gate=0;
 const ctx={book,Date,machineBusy:false,lastActivityAt:0,startTrouble,clearImageGiveUp:async()=>{},runState(){},addEvent(){},makeMachine(){},
...
test('stopped machine does not export and always releases busy flag',async()=>{
 const ctx={book:{id:'b'},Date,startTrouble,runState(){},machineBusy:false,fullAutoRunning:false,unattended:false,addEvent(){},db:{loadBook:async()=>({id:'b'})},machine:{runUntilGate:async()=>({stopped:'paused'})},halted:()=>true,finish:()=>assert.fail('exported stopped job')};
...
for(const [mode,step,expected] of [['full','done','export'],['full','gate_images','images'],[undefined,'gate_images','gate']]) {
 test(`resume preserves ${mode || 'legacy guided'} intent at ${step}`,async()=>{
  ...
  const ctx={book,machineBusy:false,hasPendingTurn:()=>false,$:()=>node,fullAutoRunning:false,clearImageGiveUp:async()=>{},

...
/**
 * ต้องกู้ธงให้จบ "ก่อน" บอกหน้าจอว่าประตูนี้รอใคร
 *
 * เดิมบล็อกกู้ธงอยู่ท้ายฟังก์ชัน แต่สองบรรทัดที่ขึ้นสถานะ "รอคุณตรวจงาน" กับส่งเสียงเรียก
 * อ่าน autoPilot() ตั้งแต่บรรทัดแรกของฟังก์ชัน เล่มที่ธงหลุดมาก่อนจึงเรียกผู้ใช้ไปแล้ว
 * ก่อนจะกู้ธงได้แล้วเดินต่อเอง — ผู้ใช้ได้ยินเสียงเรียกของงานที่ไม่เคยรอเขาเลย
 */
test('กู้ธงก่อน แล้วค่อยขึ้นสถานะรอคนกับส่งเสียงเรียก', () => {
  const restore = gate.indexOf("if (!fullAutoRunning && unattended && book?.automation?.mode === 'full')");
  const announce = gate.indexOf("runState('input', 'ตรวจต้นฉบับ");
  const call = gate.indexOf("chime('attention')");
  assert.ok(announce > 0 && call > 0, 'ต้องยังมีทั้งสถานะรอคนและเสียงเรียกอยู่');
  assert.ok(restore < announce, 'ห้ามขึ้นสถานะ "รอคุณตรวจงาน" ก่อนรู้ว่าเล่มนี้รอคนจริงไหม');
  assert.ok(restore < call, 'ห้ามส่งเสียงเรียกก่อนรู้ว่าเล่มนี้รอคนจริงไหม');
});

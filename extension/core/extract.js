...
/**
 * คำตอบที่เหลือแต่เครื่องหมายอ้างอิง — เนื้อหาจริงไม่ได้อยู่ในข้อความที่หน้าเว็บคืนมา
 *
 * เมื่อ ChatGPT ค้นเว็บก่อนตอบ ค่าที่ควรเป็นข้อความจริงจะถูกแทนด้วยหมุดอ้างอิง
 * :contentReference[oaicite:N]{index=N} ทั้งชุด — สิ่งที่อ่านกลับมาได้จึงหน้าตาแบบนี้
 *     { "titles"::contentReference[oaicite:0]{index=0}:contentReference[oaicite:1]{index=1} …
 * ล้างหมุดออกแล้วเหลือ { "titles": เปล่า ๆ เพราะเนื้อหาไม่เคยอยู่ในชั้นข้อความเลย
 *
 * ต้องแยกอาการนี้ออกจาก "คำตอบถูกตัดกลางคัน" ให้ได้ เพราะสองอย่างนี้แก้คนละทาง:
 * ตัดกลางคันแก้ด้วยการยิงซ้ำ ส่วนอันนี้ยิงซ้ำกี่รอบก็ได้ผลเดิม ต้องสั่งไม่ให้ค้นเว็บ/อ้างอิง
 * ที่ผ่านมามันถูกรายงานผิดเป็น "ตัดกลางคัน" ทำให้ตามหาสาเหตุผิดที่มาตลอด
 */
export function citationGutted(raw) {
  const s = String(raw || '');
  if (!/contentReference\[|\[oaicite:/i.test(s)) return false;
  // ล้างหมุดออกแล้วแทบไม่เหลืออะไร = เนื้อหาจริงไม่เคยมาถึงเรา
  return stripRendererNoise(s).replace(/[\s{}[\]",:]/g, '').length < 24;
}
